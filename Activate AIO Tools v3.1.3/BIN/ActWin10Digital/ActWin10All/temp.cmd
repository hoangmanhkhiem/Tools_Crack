...
cls
title  Script Kich Hoat Ban Quyen So Windows 10 Vinh Vien - Copyright (C) All rights reserved.
set Unattended=
set _args=
set _elev=
set "_arg1=%~1"
...
%ELine%
echo Phat hien phien ban OS khong duoc ho tro.
...
if "%osSKU%"=="" (
%ELine%
echo Phat hien thay SKU khong dung. Dang huy bo...
goto DL_Done
)
...
set "CLecho=Kiem tra %_1%                        [Service Status -%Cl_state%] [Startup Type -%Cl_start_type%]"
set "wlecho=Kiem tra %_2%                        [Service Status -%wl_state%] [Startup Type -%wl_start_type%]"
set "specho=Kiem tra %_3%                         [Service Status -%sp_state%] [Startup Type -%sp_start_type%]"
set "wuecho=Kiem tra %_4%                       [Service Status -%wu_state%] [Startup Type -%wu_start_type%]"
...
if defined st_s (echo Thay doi kieu khoi dong Services        [ %Clst_s%%wlst_s%%spst_s%%wust_s%] [Successful])

for %%# in (Clst_u,wlst_u,spst_u,wust_u) do if defined %%# set st_u=1
if defined st_u (call :DL_color "Loi khi thay doi kieu khoi dong         [ %Clst_u%%wlst_u%%spst_u%%wust_u%]" %Red%)
...
if defined s_s (echo Khoi dong services                      [ %Cl_s%%wl_s%%sp_s%%wu_s%] [Successful])

for %%# in (Cl_u,wl_u,sp_u,wu_u) do if defined %%# set s_u=1
if defined s_u (call :DL_color "Loi khi khoi dong services              [ %Cl_u%%wl_u%%sp_u%%wu_u%]" %Red%)

if defined wust_u (
call :DL_color "Co kha nang Service Windows Update da bi chan boi mot tool nao do. Vui long dung tool do bo chan hoac cai moi lai Windows." %Magenta%
)
...
call echo Kiem tra Product Key da cai dat         [%_channel%] [Partial Key -%%_partial%%]
)
...
set "copyfiles=Sao chep cac file duoc yeu cau vao Temp"
if defined cfailed (
...
set "GatherMod=Tao file gatherosstate sua doi         "
...
call :DL_color "Nhieu kha nang chuong trinh Antivirus da chan qua tinh nay, vo hieu hoa Antivirus roi thu lai." %Magenta%
goto :DL_Act_Cont
) else (
...
set "GenTicket=Tao GenuineTicket.xml                  "
%_noxml% (
...
set "InsTicket=Cai dat GenuineTicket.xml              "
set "TDir=%ProgramData%\Microsoft\Windows\ClipSVC\GenuineTicket"
if exist "%TDir%\*.xml" del /f /q "%TDir%\*.xml" %nul%
copy /y /b "%temp_%\GenuineTicket.xml" "%TDir%\GenuineTicket.xml" %nul%

if not exist "%TDir%\GenuineTicket.xml" (
call :DL_color "Khong sao chep duoc Ticket vao [%ProgramData%\Microsoft\Windows\ClipSVC\GenuineTicket\] Aborting..." %Red%
goto :DL_Act_Cont
)
...
echo:
set "changing_wust_back=Thay doi kieu khoi dong wu tro lai      [%wu_start_type%]"
if defined wust_s (
...
cd /d "!_work!\"
if exist "%temp_%\" @RD /S /Q "%temp_%\" %nul%
set "delFiles=Lam sach file Temp                     "
if exist "%temp_%\" (
